...
const MyExperience = () => {
  ...
  const processDevices = (devices) => {
    ...
    devices.forEach((device) => {
      if (device.isActive) {
        const deviceElement = (
          <Col key={device.deviceId} className="gutter-row" xs={12} sm={10} md={7} lg={5} xl={6}>
            <Card
              style={{
                width: 135,
                marginTop: 16,
            ...
            >
              <Meta
                ...
                description={<Badge status={device.isSynced ? "success" : "error"} text={device.deviceId} />}
              />
            </Card>
          </Col>
        );
        device.isSynced ? online.push(deviceElement) : offline.push(deviceElement);
  ...
  const handleSearch = (event) => {
    ...
    // Ensure `value` is a string before trimming or handling as an array
    const gradeFilter = Array.isArray(value)
      ? value.map((item) => item.trim()) // Handle array case
      : typeof value === "string"
      ? value.trim() // Handle string case
      : ""; // Handle null or undefined case

    setSearchValue((prevFilters) => ({
      ...prevFilters,
      [name]: gradeFilter,
    }));
  };
  ...
  useEffect(() => {
    ...
    GetMyExperience(searchValue)
      .then((res) => {
        ...
        const grades = [...new Set(res.map((item) => item.grade))].map((grade) => ({
          label: grade,
          value: grade,
        }));
  ...
  useEffect(() => {
    const applyFilters = () => {
      ...
      const filtered = experience.filter((item) =>
        Object.values(item).some((value) => value && value.toString().toLowerCase().includes(query)),
      );
  ...
  const uniqueSubjects = [...new Set(experience.map((item) => item.subject))].map((subject) => ({
    label: subject,
    value: subject,
  }));
  useEffect(() => {
            ...
            <div className="flex gap-3">
              ...
              <Select
                ...
                dropdownRender={(menu) => (
                  ...
                  >
                    <div>
                      <h4 className="m-0 mb-1 ml-3">Grade</h4>
                    ...
                    <div>
                      <h4 className="m-0 mb-1">Subject</h4>
              ...
              <Tooltip placement="top" title="Live Class">
                <Button type="primary" disabled={!deployedOpen} onClick={OpenLiveClass} className="filter-button">
                  <span className="icon">
              ...
              >
                <div className="deployment-grade-wrp bg-[#fff]">
                  {deployStep == 1 ? <img src={modal1} alt="deploy-modal" /> : <img src={modal2} alt="deploy-modal" />}
                  <div className="content">
                    ...
                    >
                      <p className="text-[24px] text-[#000] m-0 mb-3 text-center">Select the Class for Deployment?</p>
                      <p className="text-[32px] text-[#000] m-0 mb-4 text-center">
                        <span className=" text-[#00000080]">Choose</span> {deployStep === 1 ? "Grade*" : "Section*"}
                      </p>
                          ...
                          <div className="list-item-wrp">
                            {grades.map((grade) => (
                              <div key={grade.id} className="list-items">
                                {grade.name}{" "}
                                <button onClick={() => handleGradeChange(grade.id)}>
                                  <HiArrowNarrowRight />
                                </button>
                              </div>
                            ))}
              ...
              <Modal
                ...
                footer={false}
              >
                <div className="active-inactive-wrp bg-[#fff]">
                  <img src={modal2} alt="deploy-modal" />
                  <div className="active-inactive-content">
                    <p className="heading text-[#000]">Active & Inactive Devices</p>
                    <div className="flex flex-col gap-4 w-[90%]">
                      ...
                      <div className="deploy_device_status_card inactive">
                        ...
                        <div className="status">
                          <p className="text-[#000]">Inactive</p>
                          <p className="text-[#00000080]">Devices</p>
                        </div>
                        <p>{offlineCount}</p>
                      </div>
              ...
              <Modal
                ...
                footer={false}
              >
                <div className="flex flex-col gap-4 p-6 bg-[#fff]">
                  ...
                  <div className="grid grid-cols-3 gap-4 mx-3">
                    <div className="flex items-center gap-4 bg-[#F8E4F2] px-4 py-4 rounded-[20px]">
                      <div className="w-[56px] h-[56px] flex items-center justify-center bg-[#fff] rounded-full">
                        <img className="w-[35px]" src={vrgActive} alt="vr-active" />
                      </div>
                      <div className="flex flex-col">
                        <p className="text-[#000] text-[24px] font-bold m-0 p-0">{onlineDevices.length}</p>
                        <p className="text-[#000] text-[16px] m-0 p-0">Active</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 bg-[#E4F8F0] px-4 py-4 rounded-[20px]">
                      <div className="w-[56px] h-[56px] flex items-center justify-center bg-[#fff] rounded-full">
                        <img className="w-[35px]" src={vrgInactive} alt="vr-inactive" />
                      </div>
                      <div className="flex flex-col">
                        <p className="text-[#000] text-[24px] font-bold m-0 p-0">{offlineDevices.length}</p>
                        <p className="text-[#000] text-[16px] m-0 p-0">Inactive</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 bg-[#EDDEFF] px-4 py-4 rounded-[20px]">
                      <div className="w-[56px] h-[56px] flex items-center justify-center bg-[#fff] rounded-full">
                        <img className="w-[35px]" src={vrgCompleted} alt="vr-completed" />
                      </div>
                      <div className="flex flex-col">
                        <p className="text-[#000] text-[24px] font-bold m-0 p-0">{completedSyncedDevices.length}</p>
                        <p className="text-[#000] w-fit text-[16px] m-0 p-0">
                          Completed
                          <br />
                          Students
                        </p>
                          ...
                          >
                            <button
                              ...
                              className={`flex items-center gap-2 px-6 py-4 rounded-full  text-[16px]  border-none ${
                                !start ? "bg-[#ffffff1c] text-[#969696] cursor-not-allowed" : "bg-[#000] text-[#fff]"
                              }`}
                            >
                              Start{" "}
              ...
              <Tooltip placement="top" title="Add Experience">
                <Button type="primary" onClick={onRoute} className="rounded-full w-[30px] h-[30px]">
                  <PlusOutlined />
                </Button>
              </Tooltip>
            </div>
